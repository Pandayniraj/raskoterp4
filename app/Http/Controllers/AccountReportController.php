...
class AccountReportController extends Controller
{
    ...
    public function ViewLedgers()
    {
        ...
        $fiscal_year = \Request::get('fiscal_year')?  \Request::get('fiscal_year'): \App\Models\Fiscalyear::where('current_year', 1)->where('org_id',auth()->user()->org_id)->first()->fiscal_year ;
    ...
    public function paymentReport(Request $request)
    {
        ...
        $sub_groups = $type=='receipt'?(\App\Models\COAgroups::where('parent_id',\Config::get('restro.DEBTOR_LEDGER','20'))
            ->pluck('id')->toArray()):(\App\Models\COAgroups::where('parent_id',\Config::get('restro.CREDITOR_LEDGER','61'))
            ->pluck('id')->toArray());
        ...
        leftJoin($prefix.'coa_ledgers',$prefix.'coa_ledgers.id','=',$prefix.'entryitems.ledger_id')
            ...
            ->orderBy($prefix.'entries.date','desc')->where($prefix.'entries.org_id',auth()->user()->org_id);

        $bank_cash_ledger_ids=\App\Helpers\TaskHelper::getDynamicCashBankLedgerIds();

        if($request->export=='true'){
                $entryitems=$entryitems->get();
                $excel_name=$type=='receipt'?'Receipt List Report':'Payment List Report';
                return \Excel::download(new \App\Exports\PaymentReportExcel($entryitems,$bank_cash_ledger_ids,$excel_name,$startdate,$enddate), $excel_name.date('d M Y').".xls");
            }
        $entryitems=$entryitems->paginate(30);


        return view('admin.accountreport.paymentsandreceipts.payments', compact('startdate','enddate','bank_cash_ledger_ids','allFiscalYear','fiscal_year','page_title', 'page_description','tags','currency','entryitems'));
    }
        ...
        if($startdate && $enddate){
            ...
            if(!empty($request->currency)){
                $entry_items = $entry_item_table->where('ledger_id',$id)->with(['entry'])
                    ...
                    })->orderBy($prefix.'entries.date','asc')->where($prefix.'entries.org_id',auth()->user()->org_id)->orderBy($prefix.'entries.id','asc');
...
//        if ($request->start_date&&$request->end_date)
        ...
        $current_fiscal=\App\Models\Fiscalyear::where('current_year', 1)->where('org_id',auth()->user()->org_id)->first();
        $fiscal_year = \Request::get('fiscal_year')?  \Request::get('fiscal_year'): $current_fiscal->fiscal_year ;
        $fiscal=\App\Models\Fiscalyear::where('fiscal_year',$fiscal_year)->where('org_id',auth()->user()->org_id)->first();
