...
//        dd(\Request::get('payment_month'));

        if(\Request::get('year') && \Request::get('month')){

            $payroll_check = \App\Models\Payroll::where('departments_id',\Request::get('department_id'))->where('date',$year_month)->first();
            ...
            $users = User::
                select('first_name', 'last_name', 'id','emp_id')
            ...
            $tax_band = \App\Models\TaxBand::get();
...
//            dd($request->all());
            foreach($user_ids as $index=>$user)
            {
                ...
                $payment_allowances=[];
                foreach($request->allowance_value as $i=>$allowance){
                    $allowances['payroll_detail_id']=$payroll_detail->id;
                    $allowances['salary_payment_allowance_value']=$allowance;
                    $allowances['salary_payment_allowance_label']=$request->allowance_label[$i];
    ...
    public function downloadpayrollindividual(Request $request)
    {
        ...
        $payrolldetails = $payroll->payrollDetails;
        return \Excel::download(new \App\Exports\PayrollExcelExportUserFromView($user_id,$payroll,$payroll->payrollDetails), 'payroll_'.$payroll->date.'.xls');
    }
...
//        dd($slip);
//        return view('admin.payslip.pdf', compact('slip'));
