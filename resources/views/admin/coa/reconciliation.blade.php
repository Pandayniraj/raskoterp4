 ...
 function CategoryTree($parent_id=null,$sub_mark='',$ledgers_data){
    ...
    if(count($groups)>0){
    	foreach($groups as $group){
    		...
    		$ledgers= \App\Models\COALedgers::orderBy('code', 'asc')->where('group_id',$group->id)->where('org_id',\Auth::user()->org_id)->where('reconciliation','!=',0)->get();
...
<section class="content-header" style="margin-top: -35px; margin-bottom: 20px">
            <h1>
              @if(!empty($ledgers_data))  [{{$ledgers_data->code}}] {{$ledgers_data->name}} - @endif Reconcile Ledger Statement
            ...
            <div class="box-body">
							...
							<div class="row">
								<div class="col-md-6">
									...
									<label>Ledger Account</label>
										<select class="form-control select2-hidden-accessible" id="ReportLedgerId" name="ledger_id" aria-hidden="true">
					...
					<tbody>
						<tr class="bg-red">
							...
							<th>Entry Type</th>
							<th>Tag</th>
							<th>Dr.({{env(APP_CURRENCY)}})</th>
							<th>Cr.({{env(APP_CURRENCY)}})</th>
							<th>Balance ({{env(APP_CURRENCY)}})</th>
							<th>Reconcilation Date</th>
							{{-- <th></th> --}}
					    </tr>

					<form action="{{route('admin.reconcile_update')}}" method="post">
						@csrf
						  {{ method_field('PUT') }}
						 <tr class="tr-highlight">
					    	<td colspan="7">

							<span class="label label-default">Total No of enttries {{ $entry_items->count() }}</span>
							&nbsp;
					   ...
					   ?>
					    ...
					    <tr>
					    	...
					    	<td>
					    		<div class="input-group">
											<input id="ReportStartdate" type="text" name="reconciliation_date[{{$ei->id}}]" class="form-control datepicker" value="{{ $ei->reconciliation_date }}">
					                        <div class="input-group-addon">
					                            <i>
					                                <div class="fa fa-info-circle" data-toggle="tooltip" title="Note :Enter the Reconcilation Date">
					                                </div>
					                            </i>
					                        </div>
					                    </div>
				...
				<br>
				<button value="submit" type="submit" class="btn btn-primary">Reconcile</button>
					</form>
				 </div>
          </div>
      </div>
