...
<script src="/bower_components/admin-lte/select2/js/select2.min.js"></script>
 ...
 function CategoryTree($parent_id=null,$sub_mark=''){
    ...
    $groups= \App\Models\COAgroups::withCount('children')->orderBy('code', 'asc')->where('parent_id',$parent_id)->where('org_id',\Auth::user()->org_id)->get();
    if(count($groups)>0){
    	foreach($groups as $group){
            if($group->children_count>0)
    		echo '<option value="'.$group->id.'" disabled>'.'['.$group->code.']'.' '.$group->name.'</option>';
    		else
                echo '<option value="'.$group->id.'">'.'['.$group->code.']'.' '.$group->name.'</option>';
							...
							<div class="col-md-5">
								<div class="form-group">
									<label>Parent Group</label>
										<select name="group_id" id="LedgerGroupId" class="form-control" aria-hidden="true">
									   {{ CategoryTree() }}
										...
										<div class="col-md-8">
											<div class="form-group">
							                    <div class="input-group">
													<input type="number" value="" name="op_balance" class="form-control" step="any">
