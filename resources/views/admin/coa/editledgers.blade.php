    ...
    if(count($groups)>0){
    	foreach($groups as $group){
    		...
    		if($ledgers_data->group_id == $group->id){
    			...
    			echo '<option value="'.$group->id.'" Selected disabled>'.$sub_mark.'['.$group->code.']'.' '.$group->name.'</option>';
    			else
                echo '<option value="'.$group->id.'" Selected>'.$sub_mark.'['.$group->code.']'.' '.$group->name.'</option>';

    		}else{
                if($group->children_count>0)
                    echo '<option value="'.$group->id.'" disabled>'.$sub_mark.'['.$group->code.']'.' '.$group->name.'</option>';
                else
							...
							<div class="col-md-5">
								<div class="form-group">
									<label>Parent Group</label>
										<select name="group_id" id="LedgerGroupId" class="form-control" onchange="getLedgerNumber()" aria-hidden="true">
