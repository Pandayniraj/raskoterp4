...
<section class="content-header" style="margin-top: -35px; margin-bottom: 20px">
            <h1>
            <span class="material-icons">inbox </span> Meronetwork Inbox
    ...
    <div class='row'>
        	 <div class="col-md-12">
        	<a title="Reload" class="btn btn-info" href="/admin/mail/reloadinbox">Reload Inbox</a> &nbsp; <a class="btn btn-info" href="/admin/mail/sent">Sent Mails</a><br/><br/>
        </div>
        ...
        <div class="col-md-12">
          <div class="box">
            <div class="box-header with-border bg-info">
              <h3 class="box-title">Inbox <small>({{ $countmail }}) total mails. </small></h3>
                  		...
                  		@if(!\Cache::has('inbox_size'.\Auth::user()->id))
                            ...
                            @foreach ($messages as $message) 
                             ...
                             $toCache [] =  ['number'=>$message->getNumber(), 'name'=>$message->getFrom(), 'subject'=>$message->getSubject(), 'attachment'=>$message->getAttachments(), 'date'=> \Carbon\Carbon::createFromTimeStamp(strtotime($message->getDate()->format('Y-m-d H:i:s')))->diffForHumans()];  
                              ...
                              ?>
                            @endforeach
                            <?php \Cache::put('inbox_size'.\Auth::user()->id, $toCache, 1200);?>
                         @else

                         	<?php
                         		//dd( \Cache::get('inbox_size'.\Auth::user()->id));
								foreach(\Cache::get('inbox_size'.\Auth::user()->id) as $message)
								{
