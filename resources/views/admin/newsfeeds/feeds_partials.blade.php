...
@foreach($newsfeeds as $key=>$news)
 ...
 <div class="nav-tabs-custom" id='newsfeedsList-{{$news->id}}'  >
            ...
            <div class="tab-content"  @if($news->view_level == 'dept')style="background: #e4f8f987;" @endif>
                  ...
                  <div class="post ">
                     <div class="user-block">
                      ...
                      @if($news->view_level == 'dept' && $department )
                        ...
                        <span class="description" style="color:#999999;line-height: 18.5714px"> <small> {{$news->createdtime()  }}   @if($news->view_level == 'dept' && $department && $news->auto_created != '1' && $news->auto_created != '2' ) by {{$news->user->first_name  }} {{$news->user->last_name  }} @endif </small></span>
                     </div>
                    ...
                    <div class="form-group margin-bottom-none">
                      <div class="col-sm-9">
                          <textarea class="form-control input-sm post-comment" type="text" placeholder="Type a comment..." data-id = '{{$news->id}}'  id ='{{$news->id}}-postComment' ></textarea>
                      </div>
                      <div class="col-sm-3">
                        <button type="button" class="btn btn-default pull-right btn-block btn-sm post-comment" data-id = '{{ $news->id }}'>Comment</button>
                      </div>
                    </div>
                  </div>
