    ...
    <div class="box-header with-border">
        <div class='row'>
            <div class='col-md-12'>
                ...
                <div style="display: inline;">
                    <a class="btn btn-primary btn-sm" title="Create New Ticket" href="{{ route('plantransfer.create') }}">
        ...
        <div class="tab-responsive">
               ...
               @foreach ($plans as $key => $plan)
                    <tr>
                        <td>{{ $plan->TransDate}}</td>
                        <td>{{ $plan->planregister->name}}</td>
                        <td>{{ $plan->planregister->name}}</td>
                        <td>{{ $plan->ToAmount}}</td>
                        <td>
                            <a href="{!! route('plantransfer.edit', $plan->Id) !!}" title="{{ trans('general.button.edit') }}"><i class="fa fa-edit"></i></a>
                            <a href="{!! route('plantransfer.delete', $plan->Id) !!}" onclick="return confirm('Are you sure you want to delete this item?');" title="{{ trans('general.button.delete') }}"><i class="fa fa-trash deletable"></i></a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
